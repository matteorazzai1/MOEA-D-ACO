...
graph.n = length(x);
costEdge = zeros(  graph.n , graph.n );
for i=1:graph.n
    for j=1:graph.n
        if(i==j)
            costEdge(i,j)=0;
        else
            costEdge(i,j)=randi([0,10]);
            if(costEdge(i,j)==0)
                costEdge(i,j)=0.001;
            end
            costEdge(j,i)=costEdge(i,j);
        end
    end
end
...
graph.edges = zeros(  graph.n , graph.n,2 );
...
for i = 1 : graph.n
    for j = 1: graph.n
        ...
        graph.edges(i,j,1) = sqrt(  (x1 - x2) ^2 + (y1 - y2)^2  ); %to construct a matrices with a vector of 2 elements in each cell
        graph.edges(i,j,2) =  costEdge(i,j); 
